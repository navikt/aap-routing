...
@Component class Resilience4jRetryListener(registry: RetryRegistry, slacker: Slacker) {
    private val log = LoggerFactory.getLogger(Resilience4jRetryListener::class.java)

    init {
        registry.retry(GRAPHQL).eventPublisher.onRetry { event ->
            with("Retry event $event") {
                log.info(this)
