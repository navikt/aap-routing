...
class TestFordeling {
    ...
    @Test
    @DisplayName("tester at factory for hovedsøknad returnerer riktig fordeler")
    fun factoryFordelerAutomatisk() {
        whenever(arena.harAktivSak(FIKTIVTFNR)).thenReturn(false)
        whenever(arena.opprettOppgave(JP, ENHET)).thenReturn(OPPRETTET)
        with(FordelingFactory(listOf(fordeler, prodFordeler))) {
            assertThat(kanFordele(AAP, MOTTATT.name))
            assertThat(fordel(JP, ENHET).fordelingstype).isEqualTo(AUTOMATISK)
        }
    ...
    @Test
    @DisplayName("tester at hovedsøknad uten arenasak oppretter oppgave og journalfører automatisk")
    fun hovedSøknadUtenArenasak() {
        whenever(arena.harAktivSak(FIKTIVTFNR)).thenReturn(false)
        whenever(arena.opprettOppgave(JP, ENHET)).thenReturn(OPPRETTET)
        assertThat(fordeler.fordel(JP, ENHET).fordelingstype).isEqualTo(AUTOMATISK)
        verify(arena).harAktivSak(FIKTIVTFNR)
        verify(arena).opprettOppgave(JP, ENHET)
        verifyNoMoreInteractions(arena)
        verify(arkiv).oppdaterOgFerdigstillJournalpost(JP, ARENASAK)
    }

    @Test
    @DisplayName("tester at hovedsøknad med arenasak går til manuell fordeling")
    fun hovedSøknadMedArenasak() {
        whenever(arena.harAktivSak(FIKTIVTFNR)).thenReturn(true)
        assertThat(fordeler.fordel(JP, ENHET).fordelingstype).isEqualTo(MANUELL_JOURNALFØRING)
        verify(arena).harAktivSak(FIKTIVTFNR)
        verify(oppgave).opprettJournalføringOppgave(JP, ENHET)
        verifyNoInteractions(arkiv)
        verifyNoMoreInteractions(arena)
    }

    @Test
    @DisplayName("tester at manuell fordeling oppretter journalføringsoppgave når det ikke allerede finnes en")
    fun journalføringsOppgave() {
        whenever(oppgave.harOppgave(JP.journalpostId)).thenReturn(false)
        assertThat(manuellFordeler.fordel(JP, ENHET).fordelingstype).isEqualTo(MANUELL_JOURNALFØRING)
        verify(oppgave).opprettJournalføringOppgave(JP, ENHET)
    ...
    @Test
    @DisplayName("tester at manuell fordeling IKKE oppretter journalføringsoppgave når det allerede finnes en")
    fun journalføringsoppgaveFinnes() {
        whenever(oppgave.harOppgave(JP.journalpostId)).thenReturn(true)
        assertThat(manuellFordeler.fordel(JP, ENHET).fordelingstype).isEqualTo(INGEN)
        verify(oppgave).harOppgave(JP.journalpostId)
        verifyNoMoreInteractions(oppgave)
    }

    @Test
    @DisplayName("tester at manuell fordeling oppretter fordelingsoppgave når enhet ikke er kjent")
    ...
    @Test
    @DisplayName("tester at manuell fordeling oppretter fordelingsoppgave når opprettelse av journalføringsoppgave feilet")
    fun fordelingsOppgaveVedException() {
        whenever(oppgave.opprettJournalføringOppgave(JP, ENHET)).thenThrow(IrrecoverableIntegrationException::class.java)
        assertThat(manuellFordeler.fordel(JP, ENHET).fordelingstype).isEqualTo(MANUELL_FORDELING)
        verify(oppgave).harOppgave(JP.journalpostId)
        verify(oppgave).opprettFordelingOppgave(JP)
        verify(oppgave).opprettJournalføringOppgave(JP, ENHET)
        verifyNoMoreInteractions(oppgave)
    ...
    @Test
    @DisplayName("tester at manuell fordeling via factory oppretter fordelingsoppgave når opprettelse av journalføringsoppgave feilet")
    fun factory() {
        whenever(oppgave.opprettJournalføringOppgave(JP, ENHET)).thenThrow(IrrecoverableIntegrationException::class.java)
        val factory = ManuellFordelingFactory(listOf(manuellFordeler, AAPManuellFordelerProd(oppgave)))
        assertThat(factory.fordel(JP, ENHET).fordelingstype).isEqualTo(MANUELL_FORDELING)
        verify(oppgave).harOppgave(JP.journalpostId)
        verify(oppgave).opprettFordelingOppgave(JP)
        verify(oppgave).opprettJournalføringOppgave(JP, ENHET)
        verifyNoMoreInteractions(oppgave)
    ...
    companion object {
        ...
        private val JP = Journalpost(
            ...
            emptySet(), dokumenter)
        private val ENHET = NAVEnhet("666")
    }
}